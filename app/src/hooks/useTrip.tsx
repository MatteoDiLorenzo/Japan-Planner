...
function applySnapshotToReactState(
  ...
  const attrs = snap.attractionIds
    .map((id) => allAttractions.find((a) => a.id === id))
    .filter(Boolean) as Attraction[];
  setters.setActiveTripId(snap.tripId);
  setters.setTripName(snap.name);
  setters.setSelectedAttractions(attrs);
  setters.setSelectedHotels(snap.selectedHotels || []);
  setters.setCustomHotels(snap.customHotels || []);
  setters.setDayPlans(snap.dayPlans || []);
  setters.setBudgetItems(snap.budgetItems || []);
  setters.setTotalBudget(typeof snap.totalBudget === 'number' ? snap.totalBudget : 200000);
}
