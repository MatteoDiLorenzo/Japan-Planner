...
import { Map, Train, Filter, Search, Compass, Hotel, Map as MapIcon, List, X } from 'lucide-react';
...
import { formatPriceEurPrimary } from '@/lib/currency';
    ...
    <>
      ...
      <div className="flex items-center justify-between bg-white/10 rounded-xl p-4">
        ...
        <button
          ...
          aria-label={showMetro ? 'Nascondi metropolitana' : 'Mostra metropolitana'}
        >
          <div
      ...
      <div className="flex items-center justify-between bg-white/10 rounded-xl p-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-white/60 text-xs truncate">{selectedHotels.length} selezionati</p>
          </div>
        </div>
        <button
          type="button"
          ...
          aria-label={showHotels ? 'Nascondi hotel' : 'Mostra hotel'}
        >
          <div
            className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMobile && (
          <div className="flex gap-2 mb-4">
            <Button
              ...
              className="flex-1 border-white/20 text-white hover:bg-white/10 h-11 text-sm font-medium"
            >
              {showMobileList ? (
                <>
                  <MapIcon className="w-4 h-4 mr-2 shrink-0" /> Mappa
                </>
              ) : (
                <>
                  <List className="w-4 h-4 mr-2 shrink-0" /> Lista ({filteredAttractions.length})
                </>
              )}
            </Button>
            <Button
              ...
              className={`flex-1 max-w-[140px] border-white/20 text-white hover:bg-white/10 h-11 text-sm font-medium ${showFilters ? 'bg-white/10' : ''}`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
          <div className={`lg:col-span-1 space-y-4 md:space-y-6 ${isMobile && !showMobileList ? 'hidden' : ''}`}>
            ...
            {(!isMobile || showFilters) && <div className="space-y-4">{filterControls}</div>}
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          <span className="text-white/40 text-xs">{formatPriceEurPrimary(attraction.price)}</span>
                        </div>
                      </div>
                    </div>
                  </button>
          ...
          <div
            className={`lg:col-span-2 flex flex-col min-h-0 ${isMobile && showMobileList ? 'hidden' : ''}`}
          >
            <div className="flex-1 min-h-[min(72vh,calc(100dvh-200px))] h-[min(72vh,calc(100dvh-200px))] md:h-[700px] md:min-h-[700px] rounded-2xl overflow-hidden border border-white/10">
              <MapView
        ...
        {isMobile && !showMobileList && showFilters && (
          <div
            className="fixed inset-0 z-[620] flex flex-col bg-black/95 backdrop-blur-md pt-16 px-4 pb-6 overflow-y-auto"
            role="dialog"
            aria-modal="true"
            aria-label="Filtri mappa"
          >
            <div className="flex items-center justify-between mb-4 shrink-0">
              <span className="text-white font-semibold text-lg">Filtri</span>
              <Button
                type="button"
                variant="ghost"
                size="icon"
